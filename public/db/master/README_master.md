...
- **ID は原則変更しない**：既存データとの紐づけが切れるため、新規追加は新しい ID を作成する。
- **sortOrder を必ず見直す**：表示順が重要なマスタは、追加後に昇順で並ぶよう番号を再調整する。
- **重複・表記ゆれを避ける**：同義語の乱立を防ぎ、ラベルの表記を統一する（例：「有給」「有休」など）。
- **関連ドキュメントの反映**：フロント型定義（`src/types/*`）や参照コードで新規項目を使う場合は、型とUIを同期させる。
- **プレビューで確認**：変更後は `npm run lint` と本番相当のビルド／プレビューで文字化けや並び順を確認する。
...
**用途**  
求人検索・施設検索で使用するエリア階層（エリアグループ → 市区町村など）を定義。`groups` 配下の `items` が実際の選択肢。

**想定DBテーブル**  
`m_areas`（DB では親子テーブル分割を想定）
  ...
  "groups": [
    {
      ...
      "label": "熊本県内",
      "items": [
        { "id": "kumamoto_city", "label": "熊本市" },
        { "id": "yamaga_ozu_kikuyo", "label": "山鹿・大津・菊陽" }
      ]
    },
    {
      "id": "others",
      "label": "その他エリア",
      "items": [{ "id": "fukuoka", "label": "福岡県内" }]
    }
  ]
}
```
...
## holidayOptions.json（休日・シフト条件マスタ）

**用途**  
「4週8休以上」「年間休日120日以上」「週1日からOK」などの休日・シフト条件を定義。`sortOrder` で表示順を制御。
...
- 求人詳細の「休日・シフト条件」表示
- 検索条件（休日・働き方の絞り込み）
- `job_xxxx.json` の `holidayConditions.optionIds`
...
[
  {
    "id": "four_week_eight_off_or_more",
    "label": "4週8休以上",
    "sortOrder": 1
  },
  {
    "id": "annual_holidays_120_or_more",
    "label": "年間休日120日以上",
    "sortOrder": 4
  },
  { "id": "one_day_per_week_ok", "label": "週1日からOK", "sortOrder": 7 }
]
```
...
## salaryBandsHourly.json（時給レンジマスタ）

**用途** \
時給の表示レンジを定義。`min`/`max` は数値、`label` は表示文言。`max` が `null` の場合は「〜」表現で使用。

**想定DBテーブル** \
`m_salary_bands_hourly`

**主な使用箇所**

- 求人詳細の時給表示
- 検索条件の時給レンジ選択
- `job_xxxx.json` の `salary`（単位が時給のとき）
...
[
  ...
  { "id": "hourly_3000_plus", "label": "3,000円以上", "min": 3000, "max": null }
]
```
